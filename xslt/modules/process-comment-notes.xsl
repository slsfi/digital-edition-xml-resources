...
>
	...
	<!--
	XSLT Module: process-comment-notes.xsl
	Version: 1.0.0
	...
	Description:
	This module transforms one or more incoming <note> elements (which are
	in no namespace and contain minimal HTML-like markup) to TEI XML in the
	http://www.tei-c.org/ns/1.0 namespace.

	Key features:
	- All <note> elements are wrapped in <note> with updated @xml:id and @target
	  attributes (e.g., "en123" and "#start123").
	- Certain child elements (<notePosition>, <noteLemma>, <lemmaBreak>, <noteText>) 
	  become <seg> elements with @type set to their source element name.
	- Basic HTML inline elements (<em>, <i>, <sup>, <span> with @class, etc.) 
	  are transformed into TEI elements such as <hi rend="italics">,
	  <hi rend="superscript">, or custom <span>-like elements with @rend.
	- Unmatched HTML elements (e.g., <p> if no template is defined) are flattened
	  into text (on-no-match="text-only-copy"). Their text content is preserved, but
	  the element node is not retained.
	- If you wish to preserve more HTML structure, you must add more templates. 
	  For instance, to preserve <p>, define a template matching "p" to produce a 
	  <p> element in the TEI namespace.
	...
	2) In the main stylesheet, call:
	   ...
	   where $notes is a string parameter containing one or more <note> elements.
	3) The resulting transformation outputs TEI <note> elements (in the TEI namespace)
	   which can be inserted into your larger TEI document.
	...
	<!-- Declare processing mode for this module. "text-only-copy" for
	     unmatched nodes means that if there is not a template for a node/
	     an element, it is flattened, meaning that the element node is stripped
	     but the content is preserved (text) or further processed (elements). -->
	<xsl:mode name="process-comment-notes" on-no-match="text-only-copy"/>

	<!-- Apply templates to matched elements.
	     Observe that the matched elements are in no namespace, but the target
	     namespace is the TEI namespace. -->
