...
>
	...
	*
	*    Description:
	*        This XSLT module processes TEI-encoded documents and transforms
	*        <lb> elements with @break attributes and certain sibling nodes.
	*        It operates in either the `preserve-lb-breaks` or
	*        `remove-lb-breaks` mode with `on-no-match="shallow-copy"`,
	*        ensuring that unmatched nodes are copied to the output without
	*        modification.
	*
