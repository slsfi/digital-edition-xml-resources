...
>
	...
	Description:
	This XSLT module assigns sequential numbering to specific elements in TEI-encoded 
	documents, such as lines of poetry, line groups in prose, and paragraphs in
	prose. The numbering is added as an `@n` attribute to the relevant elements,
	ensuring the logical structure of the document is maintained.

	Key Features:
	- Operates in the `add-numbering` mode with `on-no-match="shallow-copy"`,
	...
	- Adds line group numbering (`<lg>`) and paragraph numbering (`<p>`) 
	  for prose texts.
	...
	using the `add-numbering` mode to execute the transformation.
	-->
	...
	<!-- <l> that are descendants of <opener>, <note> or <floatingText> are not
	numbered. <l> with @part values "M", "F" and "F2" get the same number as the
	previous <l part="I"> or <l> element. -->
	...
	<!-- Line group numbering (only in prose, together with paragraph numbering) -->
	<!-- <lg> that are descendants <opener>, <note> or <floatingText> are not
	numbered. -->
	...
	<!-- Paragraph numbering (only in prose, together with line group numbering) -->
	<!-- <p> that are descendants of <opener>, <closer>, <table>, <note> or
	<floatingText> are not numbered. -->
