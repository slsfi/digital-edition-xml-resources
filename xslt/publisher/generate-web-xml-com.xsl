...
>
	...
	*        prepares them for web publication. First comment notes
	*        provided in the input parameter `notes` are transformed to TEI
	*        XML. Then the following transformations are performed on the
	*        source document:
	*
	*        1. The processed comment notes are inserted in the
	*           <div type="notes"> element in the source document (if this
	*           element is not present, the notes will not be inserted).
	*        2. Metadata is added based on input parameters.
	*        3. @xml:space is stripped from the <body> element, so
	...
	*
	*    Usage:
	*        Set input parameters on the XSLT processor (`notes`, for other
	*        parameters see `add-metadata.xsl`) and execute the
	*        transformation on a TEI-encoded XML source document.
	*
	*    Output:
	...
	******************************************************************* -->
	...
	<xsl:template match="/">
		...
		<xsl:variable name="pass1-result">
			<xsl:apply-templates select="." mode="insert-comment-notes">
				<!-- * Pass along the processed comment notes as a
				     * parameter. * -->
	...
	<!-- * Insert the new notes into <tei:div type="notes"> in the
	     * `insert-comment-notes` mode. * -->
