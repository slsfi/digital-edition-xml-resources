...
>
	...
	Description:
	This XSLT document processes TEI-encoded manuscript documents and
	prepares them for web publication. The following transformations are
	performed on the source document:

	1. <anchor> elements related to comment notes (@xml:id starts with
	   `start` or `end`) are removed.
